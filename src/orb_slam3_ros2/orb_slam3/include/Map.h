...
public:
    ...
    KeyFrame* GetFirstKeyFrame(); // Returns the pointer to the very first KeyFrame in the Map
    ...
    KeyFrame* GetOriginKF(); // Returns the very first Keyframe of this Map which may not have had optimized pose or MapPoints
    ...
    //* 3D CUBOID
    // variables as named in CubeSLAM of object map scale
    // these maybe work variables no expalanation of important variables were given
    cv::Mat InitToGround; 
    cv::Mat GroundToInit; // orb's init camera frame to my ground
    ...
    int img_width, img_height; // no conflict, these names are in context of the Map class
...
protected:
    ...
    std::set<KeyFrame*> mspKeyFrames; // Master list of all KeyFrames inserted sequentially
