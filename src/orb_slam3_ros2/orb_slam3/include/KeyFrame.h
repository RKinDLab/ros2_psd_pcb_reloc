/**
 * @file Keyframe.h
 * @brief Definition for Keyframe.cc
...
public:
    ...
    //* Class constructors
    KeyFrame();
    KeyFrame(Frame &F, Map* pMap, KeyFrameDatabase* pKFDB); // Original constructor
    KeyFrame(Frame &F, Map *pMap); // Constructor that does not use KeyFrameDatabase
    //* Constructor overload which utilizes semantic matrix
    KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB, const eigenMatXf &passed_SemMat); // Overloaded version

    // Pose functions
    void SetPose(const Sophus::SE3f &Tcw);
    void SetVelocity(const Eigen::Vector3f &Vw_);

    Sophus::SE3f GetPose(); // mTcw
    Sophus::SE3f GetPoseInverse(); // mTwc
    Eigen::Vector3f GetCameraCenter(); // 3x1 camera center vector
    ...
    //* PSD - Pose Semantic Descriptor

    eigenMatXf maSemMat; 
    
    struct psdStruct psdKfObj; // psdKfObj is the actual PSD object.

    void ComputePSD(Frame* ptrFrame);
    void PrintDebugPSD();
    ...
    std::set<MapPoint*> GetMapPoints(); // Returns All 3D MapPoints of this Keyframe
...
public:
    ...
    long unsigned int mnId; // KeyFrame's own integer ID number
    ...
    Sophus::SE3f mTcp; // m -- matrix, T -- 4x4 homogenous pose, c -- camera, p -- parent
