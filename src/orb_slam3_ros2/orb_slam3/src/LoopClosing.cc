    ...
    mbLoopDetected(false), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mbActiveLC(bActiveLC)
{
    ...
    mnNumCorrection = 0;
    mnCorrectionGBA = 0;
} // end of LoopClosing constructor
...
// ************************************************************************************************************************************************************
//* Core method that executes loop closing in parallel
void LoopClosing::Run()
{
    mbFinished =false;

    //unsigned long int nKFBeforeLCStarts = 100; // EXPERIMENTAL

    while(1)
    {
        //* NEW LOOP AND MERGE DETECTION ALGORITHM 
        //* NEW LOOP CANDIDATE DETECTION USING SEMANTIC SPATIAL STRUCTURE
        // Heuristic guard
        if(CheckNewKeyFrames())
        {
            
            //? Why this watchdog is needed?
            if(mpLastCurrentKF)
            {
            ...
            bool bFindedRegion = NewDetectCommonRegions(); // Original method
            // bool bFindedRegion = DoLoopClosureWithSSS(mpLoopMatchedKF, mpLoopLastCurrentKF, mg2oLoopSlw, mvpLoopMPs, mvpLoopMatchedMPs); // Idea here if done correctly, the loop closure would work
        
            //bFindedRegion = false;
            //mbLoopDetected = false; // DELETE LATER

            //* This part of the code executes Loop correction
            // TODO make this into a parameter, at least a good number of keyframe is in the map before we allow loop correction
            if(bFindedRegion)
            {
                //* Multi-Session Map Merging (not needed for Paper 1)
                if(mbMergeDetected)
                {
                ...
                //* Loop Closure
                //* mbLoopDetected is set b DetectCommonRegionbyBow method later in the function
                if(mbLoopDetected)
                {
                    std::cout<<"=================================================================\n";
                    std::cout<<"mbLoopDetected TRUE at KeyFrame ID: "<<mpCurrentKF->mnId<<std::endl;
        
                    bool bGoodLoop = true; // We assume detected loop is Good Loop but why
                    ...
                    mg2oLoopScw = mg2oLoopSlw; //*mvg2oSim3LoopTcw[nCurrentIndex]; // Output from DetectCommonRegionsFromBoW
                    
                    // Watchdog: Logic for IMU based system
                    ...
                    //! VVI logic do not touch this part
                    if (bGoodLoop) {
                        mvpLoopMapPoints = mvpLoopMPs;
                        //SystemUtils::printStrMsgOneBlnk ("Running CorrectLoop!");
                        //std::cout<<"MPs in mvpLoopMapPoints: "<<mvpLoopMapPoints.size()<<std::endl; 
                        CorrectLoop(); // Perform MapPoint and Pose correction
                        std::cout<<"=================================================================\n";
                        mnNumCorrection += 1;
                    }

                    //* Reset all variables for next run
                    mpLoopLastCurrentKF->SetErase(); // Activate only with NewDetectCommonRegions() otherwise causes error code -11
                    mpLoopMatchedKF->SetErase();
                    mnLoopNumCoincidences = 0;
                    mvpLoopMatchedMPs.clear();
                    mvpLoopMPs.clear();
...
// Returns status of whether a new KeyFrame is avialable in the Local Map
{
    unique_lock<mutex> lock(mMutexLoopQueue);
...
// ************************************************************************************************************************************************************
// ORB_SLAM3's new place recognition combining Loop Candidate and Merge Candidate detections
//* Modified to use Semantically Informed Loop Closure method
bool LoopClosing::NewDetectCommonRegions()
{
    //* ORB_SLAM3's new version of detecting loop closing candidate
    mbActiveLC = true; // To deactivate placerecognition. No loopclosing nor merging will be performed
    
    if(!mbActiveLC)
        return false;
    
    //* Control whether looping closing and map merging is to work. Kept true 
    bool irosNoLoop = true; // Default false, IROS 23 we do not want to do loop closure to keep scale fixed
    bool irosNoMerge = true; // Default true, IROS 23 without merging ORB SLAM3's novel map welding will not work
    
    bool mbNormalMode = false; // Initialization
    bool mbUseSemInformedLC = false; //* Set this true to use proposed Semantically Informed Loop Closure

    // Automatically switch mode
    if (mbUseSemInformedLC){
        mbNormalMode = false; 
    }
    else{
        mbNormalMode = true; //* Activate original functionality of NewDetectCommonRegions
    }

    {
        unique_lock<mutex> lock(mMutexLoopQueue);
        mpCurrentKF = mlpLoopKeyFrameQueue.front(); // .front() returns a reference to the first element in the vector.
    ...
    std::vector<KeyFrame*> vpConnectedKeyFrames;
    ...
    //std::cout << "NewDetectCommonRegions(): Checking KF: " << mpCurrentKF->mnId << "\n" <<std::endl;

    //Check the last candidates with geometric validation
    bool bLoopDetectedInKF = false; // Is this Keyframe a loop candidate? (probably thats what its doing?)
    bool bCheckSpatial = false;
    bool mbfireLC = false;

    // TODO needs more information
    if(mnLoopNumCoincidences > 0) // original version
    {
        std::cout << "NewDetectCommonRegions(): Inside (mnLoopNumCoincidences > 0)" << "\n";
        ...
        //* Another method of Loop closure
        ...
        if(bCommonRegion)
        {
            ...
            mbLoopDetected = mnLoopNumCoincidences >= 3; // mbLoopDetected is set True if bCommonRegion is executed three times
            mnLoopNumNotFound = 0;
    ...
    //* If either mbMergeDetected or mbLoopDetected was set true from last keyframe, push current keyframe to Map
    ...
    //* Method: Semantically Informed Loop Closure [not developed]
    // Initialize
    if (mbUseSemInformedLC){
        std::vector<KeyFrame*> vpLoopCandSem;
        mpKeyFrameDB->DetectLoopCandidatesWithSemantics(mpCurrentKF, vpLoopCandSem, mpLastMap);
        mbfireLC = mpKeyFrameDB->FireLoopClosure(mpLastMap, vpLoopCandSem);
    
        // if (mbfireLC){
        //     std::cout<<"mbfireLC set true in KF ID: "<<mpCurrentKF->mnId<<std::endl;
        // }
    }
    
    // Find candidates for Merge and Loop using the Bag-of-Words and Semantics
    std::vector<KeyFrame*> vpMergeBowCand;
    std::vector<KeyFrame*> vpLoopBowCand;
    
    //* So far we did not detect either a Map Merging or detected a loop
    if((!bMergeDetectedInKF || !bLoopDetectedInKF) && (mbfireLC||mbNormalMode)) // 2nd logic comes from the Semantically Informed Loop Closure
    {
        //* Search in BoW, example of method of returning multiple outputs using aliases
        mpKeyFrameDB->DetectNBestCandidates(mpCurrentKF, vpLoopBowCand, vpMergeBowCand,3); // 3 maybe means we want to detect 3 best candidates
    }

    //* Check if the returned vpLoopBowCand list is not empty and geometric method did not detect LoopCandidate
    if(!bLoopDetectedInKF && !vpLoopBowCand.empty() && irosNoLoop)
    {
        mbLoopDetected = DetectCommonRegionsFromBoW(vpLoopBowCand, mpLoopMatchedKF, mpLoopLastCurrentKF, mg2oLoopSlw, mnLoopNumCoincidences, mvpLoopMPs, mvpLoopMatchedMPs);
    }

    //! Semantic information not tested for multi session maps
    // Merge candidates
    //if(!bMergeDetectedInKF && !vpMergeBowCand.empty())
    if(!bMergeDetectedInKF && !vpMergeBowCand.empty() && irosNoMerge)
    {
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF, g2o::Sim3 &gScw, int &nNumProjMatches,
    ...
    if(nNumProjMatches >= nProjMatches)
    {
        ...
        bool bFixedScale = mbFixScale;       // TODO CHECK; Solo para el monocular inertial (wha??)
...
// Original method that uses BoW to decide if current candidates are good keyframes
...
{
    // TODO write a one liner description
    //* Parameters
    int nBoWMatches = 20;
    int nBoWInliers = 15;
    int nSim3Inliers = 20;
    ...
    // Initialize
    set<KeyFrame*> spConnectedKeyFrames = mpCurrentKF->GetConnectedKeyFrames(); // Ka and its connected
    int nNumCovisibles = 10;

    // Define two ORB based RANSAC matcher objects
    ORBmatcher matcherBoW(0.9, true); // Original
    ...
    int index = 0;
    // std::cout<<"Candidates to check: "<<vpBowCand.size()<< " for query frame "<< mpCurrentKF->mnId <<std::endl;
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        //std::cout << "pKFi candidate: " << pKFi->mnId << std::endl;
        
        // Current KF against KF with covisibles version
        std::vector<KeyFrame*> vpCovKFi = pKFi->GetBestCovisibilityKeyFrames(10); // Gets the 10 best covisible keyframes
        if(vpCovKFi.empty())
        {
        ...
        {
            // std::cout << "Covisible list not empty" << std::endl;
            vpCovKFi.push_back(vpCovKFi[0]);
            vpCovKFi[0] = pKFi;
        }

        // WatchDog: Maybe reject checking BoW if this keyframe is too close to the matched one?
        // TODO check what happens if we disable this watchdog and directly send loop candidates
        bool bAbortByNearKF = false;
        
        //* PCB candidates almost exclusively gets rejected by this watchdog
        ...
        if(bAbortByNearKF)
        {
            //std::cout << "Check BoW aborted because query is close to the matched one " << std::endl;
            continue;
        }
        
        //std::cout << "Check BoW continues as query far from to the matched one" << std::endl;
        ...
        KeyFrame* pMostBoWMatchesKF = pKFi; // Inititalize
        int nMostBoWNumMatches = 0;

        //* Initialize vector containters
        ...
        //* Match common MapPoints between two keyframes using Bag-of-Words and RANSAC
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            if(!vpCovKFi[j] || vpCovKFi[j]->isBad())
                continue;

            //* 3D-3D Map Point Matching Keypoint matches
            int num = matcherBoW.SearchByBoW(mpCurrentKF, vpCovKFi[j], vvpMatchedMPs[j]);
            
            //std::cout<<"SearchByBow num matches between KFs ("<<mpCurrentKF->mnId<<","<< vpCovKFi[j]->mnId << "): "<< num <<std::endl;
        ...
        //* Check 3D matched map points between active keyframe and covisible keyframes of this candidate
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
        ...
        //std::cout<<"pKFi candidate: "<<pKFi->mnId<<" has "<<numBoWMatches<<" numBoWMatches"<<std::endl;

        //* Compute Tam the similarity transform
        //* This similarity transform is needed to update the whole map??
        if(numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            while(!bConverge && !bNoMore)
            {
                ...
                //std::cout << "BoW guess: Solver achieve " + to_string(nInliers) + " geometrical inliers among " + to_string(nBoWInliers) + " BoW matches" << std::endl;
            ...
            if(bConverge)
            {
                std::cout << "SolverSim3 converged" << std::endl;
                //std::cout <<"BoW guess: Convergende with " + to_string(nInliers) + " geometrical inliers among " + to_string(nBoWInliers) + " BoW matches"<<std::endl;
...
// ************************************************************************************************************************************************************
//* VERY VEYR IMPORTANT METHOD, PERFORMS THE TRUE LOOP CORRECTION AS DESCRIBED IN ORB_SLAM1 and ORB_SLAM3 paper
void LoopClosing::CorrectLoop()
{
    std::cout << "Running CorrectLoop in Frame: "<<mpCurrentKF->mnFrameId<< std::endl; // Frame ID of the good loop keyframe
    ...
    // std::cout << "LC: number of connected KFs -> " + to_string(mvpCurrentConnectedKFs.size()) << std::endl;
...
// #endif
    ...
    // Fuse duplications.
    // SearhAndFuse is a method of Loop Closing class
    ...
    {
        //! Monocular/Stereo case
        std::cout<<"\n";
        std::cout<<"............................................................................\n";
        std::cout << "LoopCorrection(): New Scale estimation: " << mg2oLoopScw.scale() << endl;
        std::cout<<"............................................................................\n";
        std::cout<<"\n";
...
// #endif
    ...
    mLastLoopKFid = mpCurrentKF->mnId; //Old variable, do not disturb it

    // IROS 23 update global scale
    nGlobalScale = mg2oLoopScw.scale();
}
// ************************************************************************************************************************************************************
