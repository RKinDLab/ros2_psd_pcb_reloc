/**
 * @file Keyframe.cpp
 * @brief Implementation of Keyfame and its related functions
...
// Original version used in ORB_SLAM3
...
/**
 * @brief Overload constructor that does not tie keyframe with a KeyFrameDatabase
 * @attention does not have a pointer to the KeyframeDatabase class
*/
KeyFrame::KeyFrame(Frame &F, Map *pMap):
    ...
    mvInvLevelSigma2(F.mvInvLevelSigma2), mnMinX(F.mnMinX), mnMinY(F.mnMinY), mnMaxX(F.mnMaxX),
    mnMaxY(F.mnMaxY), mK_(F.mK_), mPrevKF(NULL), mNextKF(NULL), mpImuPreintegrated(F.mpImuPreintegrated),
    mImuCalib(F.mImuCalib), mvpMapPoints(F.mvpMapPoints), mpORBvocabulary(F.mpORBvocabulary), mbFirstConnection(true), mpParent(NULL), mDistCoef(F.mDistCoef), mbNotErase(false), mnDataset(F.mnDataset),
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap), mbCurrentPlaceRecognition(false), mNameFile(F.mNameFile), mnMergeCorrectedForKF(0),
    mpCamera(F.mpCamera), mpCamera2(F.mpCamera2),
    mvLeftToRightMatch(F.mvLeftToRightMatch),mvRightToLeftMatch(F.mvRightToLeftMatch), mTlr(F.GetRelativePoseTlr()),
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false)
...
/**
 * @brief Overloaded constructor, uses semantic matrix
*/
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB, const eigenMatXf &passed_SemMat):
    bImu(pMap->isImuInitialized()), mnFrameId(F.mnId),  mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
    mfGridElementWidthInv(F.mfGridElementWidthInv), mfGridElementHeightInv(F.mfGridElementHeightInv),
    mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnBALocalForMerge(0),
    mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0), mnRelocWords(0), mnBAGlobalForKF(0), mnPlaceRecognitionQuery(0), mnPlaceRecognitionWords(0), mPlaceRecognitionScore(0),
    ...
    mpCamera(F.mpCamera), mpCamera2(F.mpCamera2),
    mvLeftToRightMatch(F.mvLeftToRightMatch),mvRightToLeftMatch(F.mvRightToLeftMatch), mTlr(F.GetRelativePoseTlr()),
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false), maSemMat(passed_SemMat)
{
    ...
    // Check if velocity model can be used
    if(!F.HasVelocity()) {
        mVw.setZero();
        mbHasVelocity = false;
    }
    ...
    mImuBias = F.mImuBias;
    SetPose(F.GetPose()); // This populates the mTwc variable
    mnOriginMapId = pMap->GetId();
    // Point to the frame object
}

/**
 * @brief Computes the PSD descriptor as described in IEEE AIM 2024 paper
 * @note Method has access to all global variables from Frame class
 * @note We assume the constituent Frame pose was optimized properly 
*/
void KeyFrame::ComputePSD(Frame* ptrFrame){
    //? TODO expand to include dynamic objects
    psdKfObj.kfID = mnId; // int, no pointer needed as this is global
    psdKfObj.StatObjs = ptrFrame->frameStatObjs; // std::vec<float>
    psdKfObj.L2NormStatObjs = SystemUtils::calcL2NormFromStdVecFloat(psdKfObj.StatObjs); // float
    psdKfObj.BBoxCordsStatObjs =ptrFrame->mBBoxCordsStatObjs; // std::vec<std::vec<float>>
    psdKfObj.mTwc = SystemUtils::sophusToEigenMat4f(GetPoseInverse()); // Eigen::Matrixf<Row_Major> GetPoseInverse() is a function of KeyFrame class
    psdKfObj.UnitQuat = GetPoseInverse().unit_quaternion(); //Eigen::QuaternionF object
}

/**
 * @brief Debug print the PSD data structure
*/
void KeyFrame::PrintDebugPSD(){
    std::cout << "psdKfObj.kfID: ";
    std::cout << psdKfObj.kfID << std::endl;;
    
    std::cout << "psdKfObj.StatObjs: ";
    for (auto& i: psdKfObj.StatObjs) std::cout << i << ' ';
    std::cout << "\n" << std::endl;

    std::cout << "psdKfObj.BBoxCordsStatObjs: \n";
    SystemUtils::printStdVecInVecsFloat(psdKfObj.BBoxCordsStatObjs);
    std::cout << "\n" << std::endl;

    std::cout << "psdKfObj.mTwc:\n";
    std::cout << psdKfObj.mTwc;
    std::cout << "\n" << std::endl;

    std::cout << "psdKfObj.UnitQuat: ";
    std::cout<< "q.w -- " << psdKfObj.UnitQuat.w() << " q.x -- " << psdKfObj.UnitQuat.x() << " q.y -- " << psdKfObj.UnitQuat.y() << " q.z -- " << psdKfObj.UnitQuat.z() << std::endl;
    std::cout << "\n" << std::endl;
}
...
// Returns All 3D MapPoints of this Keyframe
