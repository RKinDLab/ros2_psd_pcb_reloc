...
using namespace std; // Bring all defined names from "std", usually not a good practice

namespace ORB_SLAM3
{
    // Class constructor (original version)
    ...
    // ******************************************************************************************************************************************************
    // Modified version that requires a flag to be manually set to use the two versions
                                             ...
                                             Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
    {
        ...
        float SH, SF;
        bool H_or_F = false; // Set to true if Homography test is true
        Eigen::Matrix3f H, F;

        // Compute homography matrix using H
        ...
        //* Choose selection strategy based on provided model
        if (useKappaModel){
            //std::cout<<"TwoViewReconstruction: Selection strategy -- Kappa Model\n";
            
            // H_or_F = CheckIfHomographyOrEssentialModel(H,F,0.5,100.0); // ::CheckIfHomographyOrEssentialModel(H,F, cond_min_num, cond_max_num)
            H_or_F = CheckIfHomographyOrEssentialModel(H,F,0.5,10.0); // ::CheckIfHomographyOrEssentialModel(H,F, cond_min_num, cond_max_num)

            // * Kappa Model: Is homography matrix stable?
            if(H_or_F == 1) {
                //std::cout << "TwoViewReconstruction: Initialization from Homography\n";
                return ReconstructH(vbMatchesInliersH,H, mK,T21,vP3D,vbTriangulated,minParallax,50);
            }
            else {
                //std::cout << "TwoViewReconstruction: Initialization from Fundamental\n";
                return ReconstructF(vbMatchesInliersF,F,mK,T21,vP3D,vbTriangulated,minParallax,50);
            }

        }
        else{
            //std::cout<<"TwoViewReconstruction: Selection strategy -- ORB-SLAM`s original\n";
    ...
    // ********************************************************************************************************************************
    // Proposed model selection strategy using condition numbers
    bool TwoViewReconstruction::CheckIfHomographyOrEssentialModel(const eigenMat3f &H_mat,const eigenMat3f &F_mat, const float cond_min, const float cond_max)
    {
        
        // Initialize
        JacobiSVDMat3f svd(H_mat); // Computes SVD decomposition of H_mat but does not modify it
        
        // Compute condition number kappa2 = max(singular value) / min(singular value)
        double cond_num = svd.singularValues()(0) / svd.singularValues()(svd.singularValues().size()-1);

        //SystemUtils::printTwoBlankLines();
        //std::cout << "cond numb: " << cond_num << std::endl;

        if (cond_num >= cond_min && cond_num <= cond_max)
        {
            // Motion mostly planar, choose homography matrix
            return true;
        } else{
            // Substantial parallex, choose fundamental matrix
            return false;
        }
