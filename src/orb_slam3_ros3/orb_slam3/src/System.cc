            ...
            mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    std::cout<<"Initializing TFTN + FastMF, common to all agents\n"<<std::endl;
    
    // TODO set this camera matrix from the Camera class
    //! HARDCODED, camera matrix for ORBEC ASTRA S
    cameraK = cv::Matx33d(527.0, 0, 330.9,
                            0, 526.7, 225.9,
                            0, 0, 1);

    //! HARDCODED, camera matrix for ZED camera
    // cameraK = cv::Matx33d(262.04, 0, 322.68,
    //                         0, 262.04, 180.18,
    //                         0, 0, 1);
    tftn.setCameraIntrinsicMat(cameraK);

    std::cout<<"Initializing of TFTN + FastMF, complete\n"<<std::endl;
    // TODO ------------------------ remove in ros2_psd_pcb ---------------------
...
// TODO  ----------------------- remove these from ros2_psd_pcb_reloc
/**
 ...
 * @note Called by higher level driver class i.e. ```VSLAM``` in ```vslam.cpp```
 * @attention Version that uses PCB Relocalization + Multiagent
 * @note Version that is used by Azmyin during his Ph.D. at LSU
*/
Sophus::SE3f System::TrackRGBD_LSU(const cv::Mat &im, const cv::Mat &depthmap, 
        const double &timestep, const eigenMatXf &sem_mat,const vector<IMU::Point>& vImuMeas, 
        string filename)
{
    ...
    cv::Mat imDepthToSLAM = depthmap.clone(); // Pass an untouched preprocessed copy to SLAM
    cv::Mat surfNormals;
    cv::Mat tftnOutput;
    cv::Mat outputMatImg;   // a cv::Mat "image" of shape [resultMat.rows x resultMat.cols x 3] i.e. a pseudo RGB image 
    ...
    // Check mode changevisualizeSurfNormals
    {
        unique_lock<mutex> lock(mMutexMode);
        if(mbActivateLocalizationMode)
        {
    ...
    extractDataFromSemMat(sem_mat,vObjNameWith2DBBox, imToFeed); // Extract 2D Bounding box data from semantic matrix

    //! TODO fix this to proper RGBD pipeline
    // Tracking::GrabImageRGBD_LSU that executes PCB Relocalization + Multiagent logic
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed,timestep,sem_mat,filename);
    // DEBUG
    // std::cout <<"robot0 current position: "<<Tcw.translation().transpose() <<std::endl;
    ...
    // std::cout <<"robot0 for Frame  "<<mpTracker->mCurrentFrame.mnId<<std::endl;

    // Extract surface normal using TFTN
    
    // NSF24 DEMO: Only show TFTN output for robot0
    // TODO move this toa  separate thread
    if (mnAgentName == "robot0")
    {
        imDepthToFeed = prepDepthImageForTFTN(imDepthToFeed, mpTracker->mpubDepthMapFactor); // Common depth preprocess for both SLAM and TFTN
        tftn.runTFTN(imDepthToFeed, tftnOutput, outputMatImg, R_MEDIAN_STABLE_4_8); // run TFTN on the whole image
        surfNormals = tftnOutput.clone();
        // visualizeSurfNormals(tftnOutput);
    }
    
    // robot0 performs 
    if (mnAgentName == "robot0" && run_fast_mf)
    {
        Object2DBBox rbkairosObj; 
        nsfRbkairosObj(vObjNameWith2DBBox,surfNormals,rbkairosObj);
        visualizeObjectSurfNormalsPatch(rbkairosObj.surfNormals, rbkairosObj.objectName);
        
        // DEBUG visualize the patch surface normal
        
        std::cout <<"Firing FastMF in robot0 for Frame  "<<mpTracker->mCurrentFrame.mnId<<std::endl;
        run_fast_mf = false; // Prevent from running again
        
        fast_mf.do_denoising = true; // Control denoising
        fast_mf.show_denoised_output = false; // To show the denoised output
        // TODO in FastMF convert these to one function
        fast_mf.directionVectorMat = rbkairosObj.surfNormals.clone(); // Load surface normal map
        
        std::cout <<"Tcw in orb slam: "<<Tcw.matrix()<<std::endl;
        fast_mf.Tcw = Tcw.matrix(); // idea 1
        
        fast_mf.preProcessDirectionVectors();
        fast_mf.compute_M_mat_singleCore(); 
        fast_mf.compute_H_mat();
        // fast_mf.Idea1(); // Inital object axis aligned to camera axis
        //fast_mf.findMFRotationOnlyYaw(); // From march, does not work  
        //std::cout << "rott: \n" << fast_mf.Rstar.matrix() << std::endl;   // Final answer
        // rbkairosObj.R = fast_mf.Rstar; // Update rotation result

        // Compute Tcd_1
        // ROBOT1KF Tcd1; // work object
        // Sophus::SE3f Tc1(Tcw);  // Tc1 is the pose of robot0 during rendevouz copy constructor SOPHUS_FUNC SE3(SE3 const& other) = default;
        // robot0ComputeTcd1(rbkairosObj.R, Tc1, Tcd1); // Computes pose of robot1 in robot0
        // robot1KeyframeMap.push_back(Tcd1); // First keyframe of robot1, needed for visualization
        // mpMapDrawer->robot0StartRenderingObservedKFs = true; // Activates robot0DrawKeyframesFromRobot1() method
    }

    return Tcw; // Return pose to higher ROS level
}
...
{   
    ...
    //* 05/25/25 vslamParam.experimentName controls whether _evo style trajectories are saved
...
/**
*@brief Using the semantic matrix, for each object, find their 2D box coordinates and 
...
// TODO  ----------------------- remove these from ros2_psd_pcb_reloc
/**
 * @brief Implements processing of a depth.png image for the TFTN library.
 * @note based on CuboidDetectorSingleView::prepDepthImageForTFTN
 * Modifes the passed data, returns None
*/
cv::Mat System::prepDepthImageForTFTN(cv::Mat imDepth, double& mDepthMapFactor){
    
    if (imDepth.empty()) 
    {
        std::cerr << "WARNING: cannot read depth image, image is empty" << std::endl;
        throw std::runtime_error("Failed to read depth image.");
    }
    
    // Depth image must be 16 bit unsigned grayscale image
    if (imDepth.depth() != CV_16U) 
    {
        std::cerr << "WARNING: cannot read depth image. The image format is not 16UC1" << std::endl;
        throw std::runtime_error("The image format is not 16UC1.");
    }

    //NOTE mDepthMapFactor is initially set to 8000
    ...
    // DEBUG view
    // cv::Mat img = imDepth.clone();
    // cv::imshow("depth image", img);
    // cv::waitKey(cvWinWaitTimeVal);
    return imDepth;
}

/**
 * @brief Returns an openCV image from the surface normal map
 * @note Maps [x,y,z] components of surface normal vector to R,G,B image space
*/
cv::Mat System::generateCvImgFromSurfaceNormals(cv::Mat& resultMat){
    // Make local copies
    //cv::Mat resultMat = resMat.clone();
    cv::Mat outputMat;
    outputMat.create(resultMat.rows,resultMat.cols, CV_16UC3);
    
    for (int i = 0; i < resultMat.rows; ++ i){
        for (int j = 0; j < resultMat.cols; ++ j)
        {
        resultMat.at<cv::Vec3f>(i, j) = resultMat.at<cv::Vec3f>(i, j) / cv::norm(resultMat.at<cv::Vec3f>(i, j));
        
        if (resultMat.at<cv::Vec3f>(i, j)[2] < 0) {
            resultMat.at<cv::Vec3f>(i, j) = -resultMat.at<cv::Vec3f>(i, j);
        }
        //Blue -- Z 
        outputMat.at<cv::Vec3w>(i, j)[0] = (resultMat.at<cv::Vec3f>(i, j)[2]+1)*(65535/2.0); // Z->BLUE
        // Green -- Y
        outputMat.at<cv::Vec3w>(i, j)[1] = (resultMat.at<cv::Vec3f>(i, j)[1]+1)*(65535/2.0); // Y->GREEN
        // Red -- X
        outputMat.at<cv::Vec3w>(i, j)[2] = (resultMat.at<cv::Vec3f>(i, j)[0]+1)*(65535/2.0); // X->RED
        }
    }
...
/**
 * @brief Visualize surface normal with 2D bounding boxes of objects
 * @attention Clones, does not modify image
*/
void System::visualizeSurfNormals(cv::Mat& resMat){
    
    // Initialize
    cv::Mat resultMat = resMat.clone();
    cv::Mat imOutToFeed;
    imOutToFeed = generateCvImgFromSurfaceNormals(resultMat);

    // View
    // Make name based agent's name
    std::string win_name = mnAgentName + "_surfnormimg";
    cv::imshow(win_name, imOutToFeed);
    cv::waitKey(1); // 1e3/fps where fps = 20 from .YAML file.
}

/**
 * @brief Visualize surface normal (processed) belonging to an object
 * @attention Clones, does not modify image
 * @note Overloaded version
*/
void System::visualizeObjectSurfNormalsPatch(cv::Mat& surfIn, std::string& objName){
    
    // View
    // Make name based agent's name
    cv::Mat resultMat = surfIn.clone();
    cv::Mat newOutImg = generateCvImgFromSurfaceNormals(resultMat);
    std::string win_name = objName + "_surfnormimg_patch";
    cv::imshow(win_name, newOutImg);
    cv::waitKey(1); // 1e3/fps where fps = 20 from .YAML file. 
}

/**
*@brief Extract the Object2DBBox belonging to RBKAIROS in this timestep
*@note Only for NSF 24 demo
*@note Only works for one RBKAIROS unit visible in the scene
*@param cv::Mat snMat is the whole surface normal image and not just the patch
*/
void System::nsfRbkairosObj(std::vector<Object2DBBox>& objsVecs, cv::Mat &snMat, Object2DBBox& objOut){
    // Initialize work variables
    int x1, y1, x2, y2, obj_id;
    cv::Mat surfNormals, surfCvImage;
    
    cv::Mat resultMat = snMat.clone();
    // cv::Mat outputMat = cv::Mat::zeros(resultMat.rows, resultMat.cols, CV_32FC3);
    cv::Mat newOutImg = generateCvImgFromSurfaceNormals(resultMat);

    // DEBUG delete later
    // std::string win_name = "tempo";
    // cv::imshow(win_name, newOutImg);
    // cv::waitKey(1); // 1e3/fps where fps = 20 from .YAML file.

    // Cycle through the detections, for RBKAIROS, extract the surface normal patch and output the object
    for (auto it = objsVecs.begin(); it != objsVecs.end(); ++it){
        
        auto& element = *it; 
        obj_id = element.obj_id;
        x1 = static_cast<int>(element.x1);
        y1 = static_cast<int>(element.y1);
        x2 = x1 + element.w - 1; // Calculate bottom right x coordinate
        y2 = y1 + element.h - 1; // Calculate bottom right y coordinate
        // x2 = static_cast<int>(element.x2);
        // y2 = static_cast<int>(element.y2);

        // Ensure coordinate are within image`s bound frame
        // TODO find out why 
        x1 = std::max(0, std::min(x1, snMat.cols - 1));
        y1 = std::max(0, std::min(y1, snMat.rows - 1));
        x2 = std::max(0, std::min(x2, snMat.cols - 1));
        y2 = std::max(0, std::min(y2, snMat.rows - 1));
    
        // Build a Object2DBBox object for RBKAIROS only
        if(obj_id == 14)
            surfNormals = snMat(cv::Range(y1, y2 + 1), cv::Range(x1, x2 + 1));
            surfCvImage = newOutImg(cv::Range(y1, y2 + 1), cv::Range(x1, x2 + 1));
            objOut.objectName = "rbkairos";
            objOut.obj_id = 14;
            objOut.x1 = x1;
            objOut.x2 = x2;
            objOut.y1 = y1;
            objOut.y2 = y2;
            objOut.surfNormals = surfNormals.clone();   // 32FC3
            objOut.surfCvImage = surfCvImage.clone();   // 16UC3
    } 
}

/**
 *@brief Compute Tcd1, the frist frame in robot1's pose chain
 *
*/
void System::robot0ComputeTcd1(Eigen::Matrix3d& R_f, Sophus::SE3f& Tc1, ROBOT1KF& Tcd1){
    // Update the Tcd1 object
    float z = 0.025; // TODO modify this to be the actual depht of object in correct scale
    Tcd1.n = glob_cnt;
    
    Eigen::Matrix3f R_test;

    // ! TODO come back after carefully stuyding how to get the intial orientation figure out
    R_test = R_f.cast<float>(); 
    // R_test = Tc1.rotationMatrix() *  R_f.cast<float>(); // R_c1 * R_f, doesn't work
    // R_test =  R_f.cast<float>() * Tc1.rotationMatrix(); // R_c1 * R_f [ looks okay]
    // R_test =  Tc1.rotationMatrix() * R_f.cast<float>().inverse(); // does not work, wrong position
    // R_test =   R_f.cast<float>().inverse() * Tc1.rotationMatrix(); // also doesn't work

    Tcd1.Tcw.setRotationMatrix(R_test); // Update oritentation with estimate

    // Sanity check
    // Tcd1.Tcw.setRotationMatrix(Tc1.rotationMatrix()); // Update oritentation with estimate
    
    std::cout <<"robot0 current position: "<<Tc1.translation().transpose() << std::endl;
    Eigen::Vector3f currentTranslation = Tc1.translation();
    
    // TODO get rid of this heuristic method, RGBD data should give the correct result
    /**
     * This may involve using 3D map points that falls infront of the camera
     * such use of points will automatically take care of sign change associated with distance
     * conversely going to a full RGB-D system may solve this problem also
    */

    if(currentTranslation(2) < 0)
    {
        currentTranslation(2) = currentTranslation(2) - z; // - Z is making robot move forward in map.
    }
    else
    {
        currentTranslation(2) = currentTranslation(2) + z; // + Z movement 
    }
    
    // Update with estimated position
    Tcd1.Tcw.translation() = currentTranslation; 
    ...
    std::cout <<"Tc1: \n"<<Tc1.matrix()<<std::endl;
    std::cout <<"\n"<< std::endl;
    std::cout <<"Tcd1.Tcw: "<<Tcd1.Tcw.matrix() << std::endl;

    glob_cnt = glob_cnt + 1; // Increment global counter
}

/**
 * @brief Return the Keyframes of this agent sorted in descending order
 * @note From NSF 23 demo
*/
std::vector<ORB_SLAM3::KeyFrame*> System::GetAllKeyframesFromSystemDecending()
{
    std::vector<long unsigned int> vpKfmnIds;   
    
    std::vector<KeyFrame*> vpKFs = mpAtlas->GetAllKeyFrames();
    std::vector<KeyFrame*> vpKFs2; // Final output array
    vpKFs2.reserve(vpKFs.size());
    //sort(vpKFs.begin(), vpKFs.end()); // The third element is a comparator

    // Pull all keyframe ids first
    for (ORB_SLAM3::KeyFrame* pKF : vpKFs){
        vpKfmnIds.push_back(pKF->mnId);
    } 

    // Sort them in ascending order
    std::sort(vpKfmnIds.begin(), vpKfmnIds.end(), std::greater<int>()); // https://stackoverflow.com/questions/9025084/sorting-a-vector-in-descending-order

    // Rearrange pointer to Keyframes in ascending order
    for (long unsigned int ii : vpKfmnIds){
        // Cycle through all the keyframes
        for (ORB_SLAM3::KeyFrame* pKF : vpKFs){
            if (ii == pKF->mnId){
                // Push it into the new array
                vpKFs2.push_back(pKF);
            }
        } 
    }
    return vpKFs2;
