...
In this system, we introduced a novel multimodal keyframe descriptor dubbed **Pose-Semantic-Descriptor (PSD)**  and a new Keyframe-based Place Recognition (KPR) method called the **Pose-Class-Box (PCB)** . Using the novel KPR method, we deomstrate an accurate and computationally efficient solution to the **short-term relocalization** problem, outperforminThis dataset contains a .zip file that includes a subset of image sequences and all neural network weights as described in our recent publication. These resources are essential for running our publicly available framework, which can be found in our GitHub repository. The image sequences are selected to demonstrate various functionalities of the framework, and the included weights are pre-trained models that facilitate immediate deployment and testing. To access the framework and detailed implementation instructions, please visit our GitHub page at [GitHub Repository URL].g to the popular relocalization framework based on [DBoW2](https://github.com/dorian3d/DBoW2) which comes standard in the [ORB-SLAM3](https://github.com/UZ-SLAMLab/ORB_SLAM3) framework. At this time, the proposed framework has been only tested with the **PINHOLE** camera model.
